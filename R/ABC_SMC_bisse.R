#' @brief Using ABC approach on BiSSE to estimate diversification rates.
...
ABC_SMC_bisse <- function(
  ...
  for (i in 1:num_iterations) {
    ...
    # determine each particle in an iteration
    while (number_accepted < number_of_particles) {
      ...
      if (prior_density_function(parameters,idparsopt) > 0) {
        ...
        if ("phy" %in% names(new_sim[[1]])) {
          # to make sure the simulated data has binary states
