...
create_ref_table_musse <- function(
    save_output = TRUE,
    id = 1){
  ...
  tmp <- DivABC:::get_musse_sim_create_obs(parameters = c(0.2,0.1,0.2,0.1,0.1,0.1,0.05),
  ...
  tmp_s2 <- unlist(DivABC:::calc_ss_musse(tmp[[1]]))
  ...
  while(i <= n_sim) {
    message("sim: ", i)
    params <- prior_gen_musse (pars = rep(0.1,7),idparsopt = 1:7)
    message("params: ", params)
    # sim a tree with n_tips extant (rphylo tries to produce n extant)
    tr <- tryCatch(withTimeout({DivABC::get_musse_sim_create_obs(parameters = params,
                                                                 pool_init_states = c("1","2","3"))},
                               timeout = 50, onTimeout = "error"), error = function(e) NULL)
    if(is.null(tr)) next
    st1 <- tryCatch(treestats::calc_all_stats(tr[[1]]$phy), error = function(e) NA)
    st2 <- tryCatch(unlist(DivABC:::calc_ss_musse(tr[[1]])), error = function(e) NA)
    st <- c(st1,st2)
