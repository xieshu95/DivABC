...
library(DivABC)
param_space_name <- "DAISIE_ABC_DI_conti"
param_space <- load_param_space(param_space_name = "DAISIE_ABC_DI_conti")
lac_MLE <- c()
mu_MLE <- c()
gam_MLE <- c()
laa_MLE <- c()
...
MLE_DI_conti <- data.frame(lac_MLE, mu_MLE, gam_MLE, laa_MLE,max_ll, #K,init_K
                      init_lac,init_mu,init_gam,init_laa)

# save(MLE_all, file = paste0("/home3/p286026/results/MLE_",seed_mle,".RData"))
save(MLE_DI_conti, file = paste0("D:/Onedrive-shu/OneDrive/project 2/results/daisie-conti/DI/MLE_DI_conti.RData"))

#####
# calculate MLE with different initials (10 replicates for each parameter set)
library(DivABC)
param_space_name <- "DAISIE_ABC_DD_conti"
param_space <- load_param_space(param_space_name = "DAISIE_ABC_DD_conti")
lac_MLE <- c()
mu_MLE <- c()
gam_MLE <- c()
laa_MLE <- c()
...
for(i in 1:300) {
  ...
  while(rep < 2) {
    ...
    if(!is.na(MLE_allpars$lambda_c)) {
      ...
      laa_MLE <-c(laa_MLE,MLE_allpars$lambda_a)
      max_ll<- c(max_ll,MLE_allpars$loglik)
      K_MLE <- c(K,MLE_allpars$K)
      rep <- rep + 1
    }
  }
}
MLE_DD_conti <- data.frame(lac_MLE, mu_MLE, gam_MLE, laa_MLE,K_MLE,max_ll,init_K,
                           init_lac,init_mu,init_gam,init_laa)

# save(MLE_all, file = paste0("/home3/p286026/results/MLE_",seed_mle,".RData"))
save(MLE_DD_conti, file = paste0("D:/Onedrive-shu/OneDrive/project 2/results/daisie-conti/DD/MLE_DD_conti.RData"))
