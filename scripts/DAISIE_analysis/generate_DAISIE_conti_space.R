...
while(set < 301){
  ...
  obs_sim[[set]] <- get_DAISIE_sim_DI(parameters = c(obs_sim_pars[1],
                                           obs_sim_pars[2],
                                           obs_sim_pars[3],
                                           obs_sim_pars[4],
                                           Inf),
...
while(set < 301){
  ...
  gam <- stats::runif(1,0,0.01)
  laa <- stats::runif(1,0,1)
  K <- stats::runif(1,20,50)
  obs_sim_pars <- c(lac,mu,gam,laa,K)
  obs_sim[[set]] <- get_DAISIE_sim_DD(parameters = c(obs_sim_pars[1],
                                                     obs_sim_pars[2],
                                                     obs_sim_pars[3],
                                                     obs_sim_pars[4],
                                                     obs_sim_pars[5]),
...
colnames(pars) <- c("lac","mu","gam","laa","K")
rownames(pars) <- 1:300
DAISIE_ABC_DD <- data.frame(pars)
save(DAISIE_ABC_DD, file = "inst/extdata/DAISIE_ABC_DD_conti.rda")
DAISIE_MCMC_DD = DAISIE_ABC_DD
save(DAISIE_MCMC_DD, file = "inst/extdata/DAISIE_MCMC_DD_conti.rda")

save(obs_sim,file = paste0("inst/extdata/obs_sims_DAISIE_ABC_DD_conti.rda"))
save(obs_sim,file = paste0("inst/extdata/obs_sims_DAISIE_MCMC_DD_conti.rda"))
