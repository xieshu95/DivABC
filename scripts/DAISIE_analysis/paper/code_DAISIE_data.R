...
p <- DAISIE_plot_input(
  trees,
  age = 5,
  tcols = c(0.3, 0.8, 1.7, 2.7, 2.0, 4.2, 4.5),
)

tiff(paste0("D:/Onedrive-shu/OneDrive/project 2/DAISIE PAPER/Figures/tree3.tiff"),
     units="px", width=2500, height=1500,res = 500,compression="lzw")
print(p)
while (!is.null(dev.list()))  dev.off()
...
if (!requireNamespace('tibble', quietly = TRUE)) {
  ...
  metadata <- tibble::tibble(
    clade = names(trees),
    endemic = c(rep(FALSE,2), rep(TRUE, 5)),
    endemic_lab = if_else(endemic, "Endemic", "Non-endemic")
  )
...
tiff(paste0("D:/Onedrive-shu/OneDrive/project 2/DAISIE PAPER/Figures/tree_endemism2.tiff"),
     units="px", width=2000, height=1600,res = 500,compression="lzw")
print(p)
while (!is.null(dev.list()))  dev.off()
