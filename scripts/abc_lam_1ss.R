...
prior_dens <- function(params) {
  return(dunif(params,0,5))
}
  ...
  for (i in 1:50) {
    ...
    while (number_accepted < number_of_particles) {
      ...
      if (prior_density_function(parameters) > 0) {
        ...
        mean_df <- mean(df_stats$spec_nltt_error)
        if (mean_df > epsilon[i, 1]) {
          accept <- FALSE
          #the first step always accepts
          if (i == 1) accept <- TRUE
        }
...
save(abc,file = "G:/R/Traisie-ABC/results/abc_try1.RData")

hist(abc, breaks = seq(0, 0.03, by = 0.0005), col = "grey", main = "gamma")
abline(v = 0.02, lty = 2, col = "blue", lwd = 2)

hist(abc, breaks = seq(0, 1, by = 0.05), col = "grey", main = "Lambda")
