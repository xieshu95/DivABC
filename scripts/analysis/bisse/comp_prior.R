# Comparison between using different prior distributions
for (num_ss in c(0)){
  # formate results
  ...
  n_iteration <- c()
  for(i in 1:150){
    file_to_load <- grep(paste0("bisse_ABC_test_param_set_",i,"_ss_",num_ss,".RData"),
                         files,
                         value = TRUE,
                         fixed = TRUE)
...
# compare MCMC inference results with uniform and exponential prior distributions
param_data <- load_param_space(param_space_name = paste0("bisse_ABC_test"))
param_data <- param_data[1:150,]
...
for(i in 1:150){
  file_to_load <- grep(paste0("bisse_MCMC_test_param_set_", i,"_ss_1.RData"),
                       files,
                       value = TRUE,
                       fixed = TRUE)
  if (!identical(file_to_load, character())) {
...
for(i in 1:3){
  ...
  whole_df_ABC1 <- whole_df_ABC[(i*25000-24999):(i*25000),1:21]
  ...
  whole_df_MCMC1 <- whole_df_MCMC[(i*250050-250049):(i*250050),1:20]
  whole_df_MCMC1$ss = "MCMC Uni"
  whole_df_MCMC1$total <- rep(total, each = 5001)
  ...
  whole_df_MCMC2$total <- rep(total, each = 5001)
...
### comparison using the full posterior distribution
#####
## lam
i = 1
