library(TraisieABC)
param_space <- param_data <- load_param_space(param_space_name = paste0("secsse_ABC_test",2))
lam1_MLE<- c()
lam2_MLE <-c()
mu1_MLE <- c()
mu2_MLE <-c()
...
for(i in 1:100) {
  ...
  param_space_name <- "secsse_ABC_test2"
...
save(MLE_all, file = paste0("/home4/p290559/results/test2_MLE_secsse",seed_mle,".RData"))
